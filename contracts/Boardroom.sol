...
contract Boardroom is ShareWrapper, ContractGuard {
    ...
    modifier updateReward(address member) {
        if (member != address(0)) {
            ...
            seat.lastSnapshotIndex = laoptombSnapshotIndex();
            members[member] = seat;
        }
        _;
    }
    ...
    function laoptombSnapshotIndex() public view returns (uint256) {
        return boardroomHistory.length.sub(1);
    }

    function getLaoptombSnapshot()
        internal
        view
        returns (BoardroomSnapshot memory)
    {
        return boardroomHistory[laoptombSnapshotIndex()];
    }
    ...
    function getOPTombPrice() external view returns (uint256) {
        return treasury.getOPTombPrice();
    }

    // =========== Member getters

    function rewardPerShare() public view returns (uint256) {
        return getLaoptombSnapshot().rewardPerShare;
    }

    function earned(address member) public view returns (uint256) {
        uint256 laoptombRPS = getLaoptombSnapshot().rewardPerShare;
        uint256 storedRPS = getLastSnapshotOf(member).rewardPerShare;

        return
            balanceOf(member).mul(laoptombRPS.sub(storedRPS)).div(1e18).add(
    ...
    function claimReward() public updateReward(msg.sender) {
        ...
        if (reward > 0) {
            ...
            optomb.safeTransfer(msg.sender, reward);
        ...
        onlyOperator
    {
        ...
        uint256 prevRPS = getLaoptombSnapshot().rewardPerShare;
        ...
        optomb.safeTransferFrom(msg.sender, address(this), amount);
