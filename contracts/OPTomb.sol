...
contract OPTomb is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the genesis pool, team & airdrop
    uint256 public constant INITIAL_OPTOMB_GENESIS_POOL = 48000 ether;
    uint256 public constant INITIAL_OPTOMB_AIRDROP = 2000 ether;
    uint256 public constant INITIAL_OPTOMB_DAO = 10000 ether;
    ...
    /**
     * @notice Constructs the OPTOMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("OPTOMB", "OPTOMB") {
        // Mints 1 OPTOMB to contract creator for initial pool setup
    ...
    function _getOPTombPrice() internal view returns (uint256 _optombPrice) {
        try IOracle(optombOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("OPTomb: failed to fetch OPTOMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _OPTombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_OPTombPrice >= taxTiersTwaps[tierId]) {
    ...
    function setOPTombOracle(address _optombOracle) public onlyOperatorOrTaxOffice {
        require(_optombOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints OPTOMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of OPTOMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentOPTombPrice = _getOPTombPrice();
            currentTaxRate = _updateTaxRate(currentOPTombPrice);
            if (currentOPTombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        address _optombGenesis,
        address _optombAirdrop,
        address _optombDao
    ) external onlyOperator {
        require(!rewardPoolDistributed, "only can distribute once");
        require(_optombGenesis != address(0), "!_optombGenesis");
        require(_optombAirdrop != address(0), "!_optombAirdrop");
        require(_optombDao != address(0), "!_optombDao");
        rewardPoolDistributed = true;
        _mint(_optombGenesis, INITIAL_OPTOMB_GENESIS_POOL);
        _mint(_optombAirdrop, INITIAL_OPTOMB_AIRDROP);
        _mint(_optombDao, INITIAL_OPTOMB_DAO);
    }
